// AutoCodeToolkit.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "AutoCodeToolkit.h"
...
INT_PTR CALLBACK	MySqlCtxConfig(HWND, UINT, WPARAM, LPARAM);
                     ...
                     _In_ int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_AUTOCODETOOLKIT, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_AUTOCODETOOLKIT));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_AUTOCODETOOLKIT));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_AUTOCODETOOLKIT);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_EXIT:
			DestroyWindow(hWnd);
			break;
		case IDM_MYSQL_DATABASECONTEXTCONFIG:
			DialogBox(hInst, MAKEINTRESOURCE(IDD_MYSQL_CTX_PROPPAGE), hWnd, MySqlCtxConfig);
			break;
		case ID_MYSQL_TABLE2CODE:
			mysql_table2code(hWnd);
			break;
		default:
...
INT_PTR CALLBACK MySqlCtxConfig(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(lParam);
	switch (message)
	{
